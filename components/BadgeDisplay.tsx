    ...
    <div className="group relative flex flex-col items-center">
      ...
      {showTitle && (
        <span className={`text-[10px] mt-1 font-black text-center uppercase tracking-tighter 
          ${isArchitect ? 'text-purple-400 drop-shadow-[0_0_10px_rgba(168,85,247,1)]' : isElite ? 'text-yellow-500 animate-pulse' : 'text-neutral-500'}`}>
          {badge.name}
        </span>
      )}
      ...
      <div className={`absolute bottom-full mb-2 hidden group-hover:block z-20 w-48 border p-3 rounded-none shadow-2xl text-[10px] font-mono leading-tight
        ${isArchitect ? 'bg-black/90 border-purple-500 backdrop-blur-md shadow-purple-900/50' : isElite ? 'bg-black border-yellow-600' : 'bg-neutral-900 border-neutral-700'}`}>
        <p className={`font-black uppercase mb-1 ${isArchitect ? 'text-purple-400 drop-shadow-[0_0_5px_rgba(168,85,247,0.5)]' : isElite ? 'text-yellow-500' : 'text-white'}`}>{badge.name}</p>
